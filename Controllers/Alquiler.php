...
class Alquiler extends Controller
{
    ...
    public function registrar()
    {
        ...
        || empty($numero) || empty($precio) ||empty($abono) ||empty($fecha) ||empty($hora) || empty($documento)) {
            $msg = array('msg' => 'Todos os campos sao obrigatorios', 'icono' => 'warning');
        } else {
            ...
            if ($data > 0) {
                ...
                if ($vehiculo == 'ok') {
                    $msg = array('msg' => 'Aluguer registado com éxito', 'icono' => 'success', 'id_alquiler' => $data);
                }else{
                    $msg = array('msg' => 'Erro ao actualizar estado', 'icono' => 'error');
                }
            } else if ($data == "existe") {
                $msg = array('msg' => 'Aluguer ja esta registrado', 'icono' => 'warning');
            } else {
                $msg = array('msg' => 'Erro ao registar cliente', 'icono' => 'error');
            }
        }
    ...
    public function procesar(int $id)
    {
        if (is_numeric($id)) {
            ...
            if ($data == 'ok') {
                ...
                if ($accion == 'ok') {
                    $msg = array('msg' => 'Procesado com éxito', 'icono' => 'success');
                } else {
                    $msg = array('msg' => 'Erro', 'icono' => 'error');
                }
            }
            echo json_encode($msg, JSON_UNESCAPED_UNICODE);
        }
    ...
    public function pdfPrestamo($id)
    {
        ...
        $pdf->SetFont('Arial', 'B', 9);
        $pdf->Cell(135, 10, 'Dados do Cliente', 0, 1, 'C');
        $pdf->SetFont('Arial', '', 9);
        $pdf->Cell(40, 5, 'DOC: ' . $data['dni'], 1, 0, 'L');
        $pdf->Cell(95, 5, 'NOMBRE: ' . utf8_decode($data['nombre']), 1, 1, 'L');
        $pdf->Cell(65, 5, utf8_decode('Morada: ' . $data['direccion']), 1, 0, 'L');
        $pdf->Cell(70, 5, utf8_decode('Telefone: ' . $data['telefono']), 1, 1, 'L');
        $pdf->SetFont('Arial', 'B', 9);
        $pdf->Cell(135, 10, utf8_decode('Datos del Vehículo'), 0, 1, 'C');
        $pdf->SetFont('Arial', '', 9);
        $pdf->Cell(65, 5, utf8_decode('PLACA: ' . $data['placa']), 1, 0, 'L');
        $pdf->Cell(70, 5, utf8_decode('VEÍCULO: ' . $data['tipo']), 1, 1, 'L');
        $pdf->Cell(65, 5, utf8_decode('MODELO: ' . $data['modelo']), 1, 0, 'L');
        $pdf->Cell(70, 5, utf8_decode('CANT. DIAS: ' . $data['num_dias']), 1, 1, 'L');
        $pdf->Cell(65, 5, utf8_decode('PREÇO X DIA: ' . $data['precio_dia']), 1, 0, 'L');
        $pdf->Cell(70, 5, utf8_decode('ABONADO: ' . $data['abono']), 1, 1, 'L');
        $pdf->Cell(65, 5, utf8_decode('F. EMPRESTIMO: ' . $data['fecha_prestamo']), 1, 0, 'L');
        $pdf->Cell(70, 5, utf8_decode('F. DEVOLUÇÃO: ' . $data['fecha_devolucion']), 1, 1, 'L');
        $pdf->Ln();
        if ($data['estado'] == 0) {
            $total = 0;
        }else{
        ...
        $pdf->Cell(135, 5, utf8_decode('PENDENTE: ' . $total), 0, 1, 'C');
        $pdf->Ln();
    ...
    public function pdfAlquiler()
    {
        ...
        if (empty($alquiler)) {
            ...
            $pdf->SetFont('Arial', '', 12);
            $pdf->Cell(289, 5, 'Relatorio', 1, 1, 'C', true);
            $pdf->SetFont('Arial', '', 10);
            $pdf->SetFillColor(155, 155, 155);
            $pdf->SetTextColor(255, 255, 255);
            $pdf->Cell(30, 5, 'Doc. Garantia', 1, 0, 'L', true);
            $pdf->Cell(50, 5, 'Cliente', 1, 0, 'L', true);
            $pdf->Cell(30, 5, 'Matricula', 1, 0, 'L', true);
            $pdf->Cell(40, 5, utf8_decode('Veículo'), 1, 0, 'L', true);
            $pdf->Cell(25, 5, utf8_decode('Modelo'), 1, 0, 'L', true);
            $pdf->Cell(25, 5, 'D. Emprestimo', 1, 0, 'L', true);
            $pdf->Cell(25, 5, 'D. Entrega', 1, 0, 'L', true);
            $pdf->Cell(22, 5, utf8_decode('Quant. Dias'), 1, 0, 'L', true);
            $pdf->Cell(22, 5, utf8_decode('Preço x Dia'), 1, 0, 'L', true);
            ...
            foreach ($alquiler as $row) {
                ...
                if ($row['estado'] == 1) {
                    $pdf->SetTextColor(255, 0, 0);
                    $pdf->Cell(20, 5, 'Alugado', 1, 1, 'L');
                } else {
                    $pdf->SetTextColor(0, 255, 0);
                    $pdf->Cell(20, 5, 'Devolvido', 1, 1, 'L');
                }
                $pdf->SetTextColor(0, 0, 0);
            }
            $pdf->Output();
